...
def gen_single_landmark_mask(ref_image, landmark_df, spacing, pos_upper_bound, neg_lower_bound):
  ...
  print("ref_image_size", ref_image_size)
  ...
  for landmark_name in landmark_df.keys():
    ...
    print("landmark_world", landmark_world)
    landmark_voxel = ref_image.TransformPhysicalPointToIndex(landmark_world)
    print("landmark_voxcel_coordinat", landmark_voxel)
...
def gen_landmark_mask_batch(image_folder, landmark_folder, target_landmark_label,
  ...
  for image_name in image_names:
    print(image_name)
    target_landmark_df = {}
    landmark_df = pd.read_csv(os.path.join(landmark_folder, '{}.csv'.format(image_name.strip().replace(" ", ""))), header=0)
    landmark_df.columns = landmark_df.columns.str.strip()
    print(landmark_df)
    landmark_df['name'] = landmark_df['name'].str.strip()
    # print("landmark_df['name']", landmark_df['name'])
    # print("target_landmark_label.keys()", target_landmark_label.keys())
    cleaned_target_landmark_label = {key.strip(): value for key, value in target_landmark_label.items()}
    for landmark_name in cleaned_target_landmark_label.keys():
      landmark_label = cleaned_target_landmark_label[landmark_name]
      x = landmark_df[landmark_df['name'].str.strip() == landmark_name]['x'].values[0]
      y = landmark_df[landmark_df['name'].str.strip() == landmark_name]['y'].values[0]
      z = landmark_df[landmark_df['name'].str.strip() == landmark_name]['z'].values[0]
    ...
    image =sitk.ReadImage(os.path.join(image_folder, image_name +'.nii.gz'))
...
def main():
  ...
  default_batch_idx = 1
  default_input = r"C:\Users\MarwaABDERRAHIM\OneDrive - ABYS MEDICAL\Bureau\Landmarks\test_data2\data_v2"
  default_landmark = r"C:\Users\MarwaABDERRAHIM\OneDrive - ABYS MEDICAL\Bureau\Landmarks\test_data2\landmark_v2"
  default_output = r"C:\Users\MarwaABDERRAHIM\OneDrive - ABYS MEDICAL\Bureau\Landmarks\test_data2\landmark_mask_v2/batch_{}_1.5mm".format(default_batch_idx)
  default_label = r"C:\Users\MarwaABDERRAHIM\OneDrive - ABYS MEDICAL\Bureau\Landmarks\test_data2\landmark_label_file_batch_1.csv"
  default_spacing = [3, 3, 3]
  # default_spacing = [2, 2, 2]
  default_pos_upper_bound = 6
  default_neg_lower_bound = 12
